...
- [~] **Sprint 3 — CMS + Database + Auth (IN PROGRESS):** `@gocsa/cms` security core **built + 100%-tested** — reusable RBAC access layer (8 roles × lane × site, DEC-027), publishing workflow state machine (draft→in-review→changes-requested→approved→published→archived, illegal transitions rejected), validation gates (alt/consent/link/form-consent), EN/EL localisation + parity. Shared field groups (seo/cta/link/address), 15 page-builder blocks (schema), foundational + representative collections (users/media/service-groups/services/funding/faqs/testimonials/downloads/pages) + globals (settings/nav/footer). **47 CMS tests; 166 total; workspace green.** **`apps/web` now real** (DEC-026 ratified: Payload 3.86 + Next 15.4 + React 19): postgres adapter, admin, EN/EL localisation, drafts/versions, upload, access wired from the tested policy, seed, migrations dir, health + public `/[locale]` shell, integration tests, CI `cms-runtime` Postgres job. **Verified in sandbox:** typecheck 0, lint 0, `payload generate:types`, **`next build` ✓**. **Not executable here (R16, no Docker/Postgres):** migrate/seed/login/integration — run via docs/24 or CI. **Sprint 3 not marked complete until CI/DB confirms runtime.** Remaining collections (resources/news/events/staff/policies/careers/forms/redirects) follow the built pattern.
